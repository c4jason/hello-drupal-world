...
+ The requirements did not specify whether this is distinct and separte from the list of node tiles meeting criteria 2. From the screenshot, it appears within a div containing the node list below, so I implemented it as a view block title.
...
+ I accomplished this using the CSS pseudo before element on the article.node element.
+ In my opinion, "Contact starts here!" does not add substative content to the page, and merely servers as a visual indicator for the sighted user.
...
+ If I required to add "Content starts here!" as content, I would favor adding a text field to the content type, default to "Content starts here!" and dragging into the correct rendering position within the node's content. I tend to favor adding anything a client may want to alter through the UI, so it's more accessible.
